...
class FluRectangle(QQuickPaintedItem):
    ...
    def paint(self, painter: QPainter):
        ...
        path.arcTo(
            QRectF(
                QPointF(rect.topRight() - QPointF(radius[1] * 2, 0)),
        ...
        path.arcTo(
            QRectF(QPointF(rect.topLeft()), QSize(radius[0] * 2, radius[0] * 2)), 90, 90
        )
        path.lineTo(rect.bottomLeft() - QPointF(0, radius[3]))
        path.arcTo(
            QRectF(
                QPointF(rect.bottomLeft() - QPointF(0, radius[3] * 2)),
                QSize(radius[3] * 2, radius[3] * 2),
            ),
            180,
            90,
        )
        ...
        path.arcTo(
            QRectF(
                QPointF(rect.bottomRight() - QPointF(radius[2] * 2, radius[2] * 2)),
                QSize(radius[2] * 2, radius[2] * 2),
            ),
            270,
            90,
        )
