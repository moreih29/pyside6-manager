...
class FluFrameless(QQuickItem, QAbstractNativeEventFilter):
    ...
    @appbar.setter
    def appbar(self, value: QQuickItem):
        self._appbar = value
    ...
    def componentComplete(self):
        ...
        if Tools.isWin():
            ...
            if self._fixSize:
                ...
                SetWindowLongPtrW(
                    hwnd, -16, style | 0x00010000 | 0x00040000 | 0x00C00000
                )
            SetWindowPos(
    ...
    def nativeEventFilter(self, eventType: ..., message: int | None):
        ...
        if uMsg == 0x0046:
            ...
            if altPressed and spacePressed:
                pos = self.window().position()
                self._showSystemMenu(
                    QPoint(pos.x(), int(pos.y() + self._appbar.height()))
                )
        return False
    ...
    def eventFilter(self, watched: ..., event: ...):
        if self.window() is not None:  # pyright: ignore[reportUnnecessaryComparison]
            if event.type() == QEvent.Type.MouseButtonPress:
                ...
                ) and self._margins <= p.y() <= (
                    self.window().height() - self._margins
                ):
                    if self._edges != 0:
                        self._edges = 0
                        self._updateCursor(self._edges)
