...
*   **`ScaleAnimation`**: 아이템의 `scale` 프로퍼티를 애니메이션합니다.
*   **`XAnimation`, `YAnimation`**: 아이템의 `x`, `y` 프로퍼티를 애니메이션합니다.

**주요 프로퍼티 (PropertyAnimation 기준):**

| 이름         | 타입      | 기본값 | 설명                                                                                                                                 |
| :----------- | :-------- | :----- | :----------------------------------------------------------------------------------------------------------------------------------- |
| `target`     | `Object`  | `null` | 애니메이션을 적용할 대상 객체 (일반적으로 `Item`의 `id`). `Transition` 내에서는 자동으로 설정되는 경우가 많습니다.                               |
...
| `alwaysRunToEnd`| `bool` | `false`| `true`로 설정하면 `stop()` 메서드가 호출되어도 애니메이션이 즉시 멈추지 않고 현재 루프의 끝까지 실행된 후 멈춥니다.                                 |
...
*   `animations`: (읽기 전용) 그룹에 포함된 애니메이션 객체 리스트.
*   `loops`: 그룹 전체의 반복 횟수.
*   `running`: 그룹 전체의 실행 상태.
...
2.  **`Behavior` 내에서 사용**: 특정 프로퍼티 값이 변경될 때마다 자동으로 실행됩니다. (Behavior 문서 참고)
    ...
    ```
3.  **독립적인 애니메이션 객체로 사용**: `id`를 부여하고 `start()`, `stop()`, `pause()`, `resume()` 메서드를 호출하여 직접 제어합니다. 특정 이벤트 핸들러(예: `onClicked`) 내에서 애니메이션을 시작할 때 유용합니다.
    ```qml
    Rectangle {
        id: rect; width: 50; height: 50; color: "blue"
        ...
        MouseArea {
            anchors.fill: parent
            onClicked: moveAnimation.start() // 클릭 시 애니메이션 시작
        }
    }
    ```
...
| 이름        | 파라미터 | 설명                                      |
| :---------- | :------- | :---------------------------------------- |
| `started()` | -        | 애니메이션이 시작될 때 발생합니다.          |
| `stopped()` | -        | 애니메이션이 멈췄을 때 발생합니다.          |
| `paused()`  | -        | 애니메이션이 일시 정지될 때 발생합니다.      |
| `resumed()` | -        | 애니메이션이 다시 시작될 때 발생합니다.      |
| `runningChanged` | `bool`| `running` 프로퍼티 값이 변경될 때 발생합니다. |
| `alwaysRunToEndChanged` | `bool` | `alwaysRunToEnd` 프로퍼티 값이 변경될 때 발생합니다. |
...
| `complete()`| -       | -        | 애니메이션을 즉시 완료 상태로 이동합니다. |
